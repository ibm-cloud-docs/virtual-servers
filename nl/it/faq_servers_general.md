...
Se non puoi collegarti alla console KVM, controlla i seguenti suggerimenti sulla risoluzione dei problemi come supporto per la risoluzione del problema. Se si verificano ulteriori problemi, contatta il supporto. Per ulteriori informazioni su come contattare il supporto, vedi [Come ottenere aiuto e supporto](../vsi/vsi_ts_index.html).
   ...
   * La console KVM può generare una o più finestre a comparsa durante il processo di connessione. Abilita le finestre a comparsa dal {{site.data.keyword.slportal}} per garantire di poter stabilire una connessione.
   ...
   * Se sono state rispettate le seguenti condizioni e ricevi un messaggio di errore che afferma, "Manca il manifest delle autorizzazioni richiesto in main.jar", le applet Java non sono state abilitate nel pannello di controllo Java. Queste impostazioni sono state introdotte come una precauzione di sicurezza da Oracle in Java SE v7. Abilita le applet nel pannello di controllo per risolvere questo problema.
     ...
     **Nota:** se si sta utilizzando Mac OSX insieme a Google Chrome, fare riferimento a Informazioni e requisiti del sistema per l'installazione e l'utilizzo di Mac Java 7 nel sito web Java.
