...
Les serveurs virtuels transitoires sont par nature éphémères et peuvent être arrêtés à tout moment, ce qui peut entraîner une perte de données. Des notifications de récupération automatisée peuvent s'avérer utiles dans ce cas. Lors de sa mise à disposition, un serveur virtuel transitoire peut être configuré pour recevoir une notification indiquant qu'il est arrêté, **deux minutes** avant son arrêt effectif. La notification vous permet d'alerter, via un programme, le serveur virtuel transitoire et de lui demander d'arrêter le traitement en cours d'exécution ou de transférer toutes les données nécessaires en dehors du serveur virtuel transitoire.
...
2. Configurez la fonction de rappel webhook 
3. Vérifiez les demandes webhook
...
Pour configurer la fonction webhook, vous devez affecter les paramètres suivants à l'instance de serveur virtuel transitoire en utilisant l'API SLDN :
   ...
   * **Secret** - Chaîne qui est utilisée comme clé dans un algorithme de hachage pour signer la demande. Ne communiquez à personne d'autre la chaîne secrète.
...
Pour plus d'informations, voir la documentation relative à la méthode d'API SLDN pour la configuration webhook [ ![Icône de lien externe](../icons/launch-glyph.svg "Icône de lien externe")](http://sldn.softlayer.com/reference/services/SoftLayer_Virtual_Guest/setTransientWebhook/){: new_window}.
...
2. Nonce trouvé dans l'en-tête "X-IBM-Nonce" de la demande
   ...
   Il s'agit d'une chaîne qui est générée de façon aléatoire quand la demande est envoyée. Vous pouvez choisir de stocker les nonces reçus pour les comparer au nonce inclus dans la demande. Si le nonce de la demande a été utilisé auparavant, n'acceptez pas la demande, afin de prévenir tout risque d'attaque par réinsertion.

3. Code HMAC (Hash Message Authentication Code) figurant dans l'en-tête "Authorization" de la demande

   Il s'agit d'une chaîne qui a été hachée via l'algorithme HMAC-SHA256, en utilisant en tant que clé le chaîne secrète fournie, puis codée en Base64. Vous devez construire la chaîne, la hacher, la coder en Base64 puis comparer le résultat à la signature de l'en-tête "Authorization". En cas de non correspondance, n'acceptez pas la demande. Voir ci-dessous les détails relatifs à la création de la signature HMAC.
...
1. Créez la chaîne canonique.
  ...
  * Contenu - corps de la demande. Cet exemple suppose que la chaîne JSON a été décodée dans un tableau associatif natif ou dans un dictionnaire.  
  * Nonce - se trouve dans l'en-tête "X-IBM-Nonce"

  Pour créer la chaîne canonique, associez les données ci-dessus dans l'ordre EXACT ci-après, sans délimiteurs :

  Type de méthode + Type de contenu + Contenu['id'] + Contenu['serviceName'] + Contenu['event'] + Contenu['timestamp'] + Nonce

2. Hachez la chaîne canonique.

  L'algorithme utilisé dans la fonction de rappel du serveur virtuel transitoire est HMAC-SHA256. Il s'agit d'un algorithme de hachage avec clé. La clé à utiliser est le secret qui a été fourni quand la fonction webhook a été configurée.
...
La demande envoyée depuis la fonction de rappel du serveur virtuel transitoire est similaire à tout demande HTTP, en ce sens qu'elle dispose d'en-têtes et d'un contenu. Le contenu est une chaîne au format JSON, représentée ci-après :

**REMARQUE** : il n'est pas garanti que les noms de clé se présenteront dans cet ordre spécifique.
